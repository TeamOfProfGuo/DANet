...
class Options():
    ...
    def parse(self):
        ...
        args = self.parser.parse_args(['--batch-size', '2',
                                       '--model_path', './runs/pascal_voc/deeplab/resnet50/default/checkpoint.pth.tar'])
...
validate(args, evaluator, testloader, metrics)

def validate(args, evaluator, loader, metrics):
    ...
    for i, (image, dst) in enumerate(tbar):
        if args.eval:
            ...
            with torch.no_grad():
                ...
                predicts = [loader.make_pred(torch.max(output, 1)[1].cpu().numpy()) for output in outputs]
