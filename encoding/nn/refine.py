...
class MultiScaleAtt(nn.Module):
    def __init__(self, out_feats, shapes, att_type='AT1', with_bn=False):
        ...
        if att_type == 'AT1':
            ...
            self.att_module = AttBlock2(in_ch=out_feats, r=16, with_bn=with_bn)
    ...
    def forward(self, *xs):
        ...
        if self.att_type == 'AT1':
            output = self.att_module(g=out0, x=out1) + out0
        elif self.att_type == 'AT2':
...
class AttBlock2(nn.Module):
    def __init__(self, in_ch, r=16, with_bn=False):
        super().__init__()
