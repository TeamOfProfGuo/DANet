...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None,
        ...
        self.pretrained_rgb = get_backbone(backbone, pretrained=True, dilated=dilated,
                                       norm_layer=norm_layer, root=root, dim = 3,
                                       *args, **kwargs)
        self.pretrained_dep = get_backbone(backbone, pretrained=True, dilated=dilated,
                                       norm_layer=norm_layer, root=root, dim = 1,
                                       *args, **kwargs)
        self.pretrained_rgb.fc = None
        self._up_kwargs = up_kwargs

    def base_forward(self, x, branch = 'rgb'):
        if self.backbone.startswith('wideresnet'):
            x = self.pretrained_rgb.mod1(x)
            x = self.pretrained_rgb.pool2(x)
            x = self.pretrained_rgb.mod2(x)
            x = self.pretrained_rgb.pool3(x)
            x = self.pretrained_rgb.mod3(x)
            x = self.pretrained_rgb.mod4(x)
            x = self.pretrained_rgb.mod5(x)
            c3 = x.clone()
            x = self.pretrained_rgb.mod6(x)
            x = self.pretrained_rgb.mod7(x)
            x = self.pretrained_rgb.bn_out(x)
            return None, None, c3, x
        else:
            if branch == 'rgb':
                x = self.pretrained_rgb.conv1(x)
                x = self.pretrained_rgb.bn1(x)
                x = self.pretrained_rgb.relu(x)
                x = self.pretrained_rgb.maxpool(x)
                c1 = self.pretrained_rgb.layer1(x)
                c2 = self.pretrained_rgb.layer2(c1)
                c3 = self.pretrained_rgb.layer3(c2)
                c4 = self.pretrained_rgb.layer4(c3)
                return c1, c2, c3, c4
            elif branch == 'dep':
                x = self.pretrained_dep.conv1(x)
                x = self.pretrained_dep.bn1(x)
                x = self.pretrained_dep.relu(x)
                x = self.pretrained_dep.maxpool(x)
                c1 = self.pretrained_dep.layer1(x)
                c2 = self.pretrained_dep.layer2(c1)
                c3 = self.pretrained_dep.layer3(c2)
                c4 = self.pretrained_dep.layer4(c3)
                return c1, c2, c3, c4
            else:
