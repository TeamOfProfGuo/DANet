...
def get_backbone(name, **kwargs):
    ...
    #     net.load_state_dict(torch.load(get_model_file('psp_%s_%s'%(backbone, acronyms[dataset]), root=root)))
...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None,
                 ...
                 std=[.229, .224, .225], root='~/.encoding/models', # dep_dim=False, 
                 *args, **kwargs):
        ...
        self.pretrained = get_backbone(backbone, pretrained=True, dilated=dilated,
                                       norm_layer=norm_layer, root=root, dep_dim=False,
                                       *args, **kwargs)
        self.pretrained_dep = get_backbone(backbone, pretrained=True, dilated=dilated,
                                       norm_layer=norm_layer, root=root, dep_dim=True,
                                       *args, **kwargs)
