...
class PSP(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.head = PSPHead(4096, nclass, norm_layer, self._up_kwargs)
        if aux:
            self.auxlayer = FCNHead(1024, nclass, norm_layer)

    def forward(self, x):
        b, _, h, w = x.size()
        _, _, c3, c4 = self.base_forward(x[:, :3, :, :], branch='rgb')
        # _, _, c3_dep, c4_dep = self.base_forward(x[:, 3:, :, :].expand(b, 3, h, w), branch='dep')
        _, _, c3_dep, c4_dep = self.base_forward(x[:, 3:, :, :], branch='dep')

        # print('[Size] c4 = %s, c4_dep = %s' % (c4.size(), c4_dep.size()))

        c4_with_dep = torch.cat((c4, c4_dep), 1)

        outputs = []
        x = self.head(c4_with_dep)
        x = interpolate(x, (h,w), **self._up_kwargs)
        outputs.append(x)
        if self.aux:
            c3_with_dep = torch.cat((c3, c3_dep), 1)
            auxout = self.auxlayer(c3_with_dep)
...
def get_psp_db(dataset='pascal_voc', backbone='resnet50s', pretrained=False,
