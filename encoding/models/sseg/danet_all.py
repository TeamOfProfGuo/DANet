...
__all__ = ['DANet', 'get_danet_all']

class DANet_all(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(DANet_all, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
...
def get_danet_all(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = DANet_all(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
