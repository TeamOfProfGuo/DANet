...
class RUNet(nn.Module):
    ...
    def forward(self, x, d):
        ...
        c = self.fuse4(l4, d4)    # [B, 512, h/32, w/32]

        y4 = self.up4(c)              # [B, 256, h/16, w/16]
        c3 = self.fuse3(l3, d3)        # [B, 256, h/16, w/16]
        y3 = self.level_fuse3(y4, c3)  # [B, 256, h/16, w/16]

        y3 = self.up3(y3)              # [B, 128, h/8, w/8]
        c2 = self.fuse2(l2, d2)        # [B, 128, h/8, w/8]
        y2 = self.level_fuse2(y3, c2)  # [B, 128, h/8, w/8]

        y2 = self.up2(y2)              # [B, 64, h/4, w/4]
        c1 = self.fuse1(l1, d1)        # [B, 64, h/4, w/4]
        y1 = self.level_fuse1(y2, c1)  # [B, 64, h/4, w/4]
...
def get_runet(dataset='nyud', backbone='resnet18', pretrained=True, root='./encoding/models/pretrain',
               mmf_att=None, mrf_att=None ):
    from ...datasets import datasets
    model = RUNet(datasets[dataset.lower()].NUM_CLASS, backbone, pretrained, root=root,
...
class RGBDFuse(nn.Module):
    def __init__(self, in_ch, mmf_att=None):
        super().__init__()
        self.cbr = BasicBlock(in_ch, in_ch)
