...
__all__ = ['DANet_with_lamb', 'get_danet_with_lamb']

class DANet_with_lamb(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(DANet_with_lamb, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
...
class DANetHead(nn.Module):
    ...
    def forward(self, x):
        ...
        feat_sum = sa_conv - self.lamb * sc_conv
...
def get_danet_with_lamb(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = DANet_with_lamb(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
